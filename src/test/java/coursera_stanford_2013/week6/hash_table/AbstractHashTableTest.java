...
public class AbstractHashTableTest {
    ...
    protected void addAllElements(Integer[] givenValues) {
        for (int i = 0; i < givenValues.length; ++i)
            hashTable.put(i, givenValues[i]);
    }

    protected Integer[] getAllElements(Integer[] givenValues) {
    ...
    protected void assertTwoArraysAreEqual(Integer[] givenValues, Integer[] retrievedValues) {
        for (int i = 0; i < givenValues.length; ++i)
